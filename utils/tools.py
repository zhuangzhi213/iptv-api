...
def convert_to_m3u(first_channel_name=None):
    ...
    if os.path.exists(user_final_file):
        with open(user_final_file, "r", encoding="utf-8") as file:
            m3u_output = '#EXTM3U x-tvg-url="https://gh.monlor.com/https://raw.githubusercontent.com/fanmingming/live/main/e.xml"\n'
            current_group = None
            for line in file:
                    ...
                    if "#genre#" in trimmed_line:
                        ...
                        processed_channel_name = re.sub(
                            ...
                            lambda m: f"{m.group(1)}{m.group(2)}"
                                      + ("+" if m.group(3) else ""),
                            first_channel_name if current_group == "更新时间" else original_channel_name,
                        )
                        m3u_output += f'#EXTINF:-1 tvg-name="{processed_channel_name}" tvg-logo="https://tv-icon.zhuangzhi.us.kg/{processed_channel_name}.png"'
                        if current_group:
